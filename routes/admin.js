...
// A dedicated endpoint for the Audit Log tab — it used to piggyback on
// /admin/summary just to get recentActivity, which meant recomputing the
// entire occupancy/tenant-count dashboard just to show a list of log
// lines. This does only the one query that tab actually needs, and
// returns more history (50 rows) than the dashboard's compact 10.
router.get('/audit', requireAuth, withOrgScope, requireAdmin, async (req, res) => {
  try {
    const recentActivity = await db.recentAuditLog(req.db, req.user.organizationId, 50);
