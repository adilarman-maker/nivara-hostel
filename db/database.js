...
async function phoneTakenAnywhere(client, organizationId, phone, exclude = {}) {
  const { rows } = await client.query(
    `SELECT 1 FROM tenants WHERE organization_id = $1 AND phone = $2 AND ($3::text IS NULL OR id != $3)
     UNION
...
async function tenantCountsByBlock(client, organizationId) {
  const { rows } = await client.query(
    `SELECT LEFT(uid, 1) AS block, COUNT(*)::int AS n FROM tenants WHERE organization_id = $1 GROUP BY LEFT(uid, 1)`,
    [organizationId]
  );
  const counts = {};
...
async function approveProof(client, organizationId, id, finalAmount, reviewerName) {
  ...
  const { rows } = await client.query(
    `UPDATE payment_proofs SET status = 'approved', claimed_amount = $3, reviewed_at = now(), reviewed_by = $4
     WHERE organization_id = $1 AND id = $2 RETURNING *`,
